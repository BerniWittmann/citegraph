...
describe('components/visualizations/edit/parameters/NetworkParameters.vue', () => {
  const getWrapper = (vis: Visualization = new NetworkVisualization({ name: 'Test' })) => {
    return shallowMount(NetworkParameters, {
      i18n,
      mocks: {
        $t: (key: string) => key
      },
