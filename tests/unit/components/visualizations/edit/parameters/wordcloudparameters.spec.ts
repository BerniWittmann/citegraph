...
describe('components/visualizations/edit/parameters/WordCloudParameters.vue', () => {
  const getWrapper = (vis: Visualization = new WordCloudVisualization({ name: 'Test' })) => {
    return shallowMount(WordCloudParameters, {
      i18n,
      mocks: {
        $t: (key: string) => key
      },
